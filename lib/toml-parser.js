...
function makeParserClass(Parser) {
	class TOMLParser extends Parser {
		...
		recordAssignStatement(kv) {
			let target = this.ctx;
			const finalKey = kv.key.pop();
			for (const kw of kv.key) {
				if (
					hasKey(target, kw) &&
		...
		parseTableMore(keyword) {
			if (this.char === CHAR_SP || this.char === CTRL_I) {
				...
				if (
					...
					(!isTable(this.ctx[keyword]) ||
						this.ctx[keyword][_declared])
				) {
					throw this.error(
						new TomlError("Can't redefine existing key"),
					);
		...
		parseInf2() {
			if (this.char === CHAR_f) {
				if (this.state.buf === "-") {
					return this.return(Number.NEGATIVE_INFINITY);
				} else {
					return this.return(Number.POSITIVE_INFINITY);
				}
			} else {
				throw this.error(
					new TomlError(
		...
		parseNumberExponentSign() {
			if (isDigit(this.char)) {
				...
				throw this.error(
					new TomlError(
						"Unexpected character, expected -, + or digit",
					),
				);
			}
		}
